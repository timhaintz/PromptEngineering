...
# Add missing AzureGPTClient class definition
class AzureGPTClient:
    def __init__(self, client, model_version, model_config, temperature=1.0, debug=False):
        self.client = client
...
As a Senior PhD researcher and writer, your task is to review the original sentences and paragraphs, 
...
Use concise language suitable for a top-level world-class journal or publication, adhering to Australian English standards. 
...
\label{sec:beyond}
%1 - Write long introduction to logic - text
Beyond logic encourages LLMs to transcend conventional topic boundaries, fostering exploration, innovation, and forward-thinking responses. This logic is essential for tasks that require the model to hypothesise, reason, predict, or simulate scenarios beyond established knowledge or current constraints.
...
class AzureGPTClient:
    ...
    def start_chat(self, system_message: str, initial_conversation: Optional[List[Dict[str, str]]] = None):
        ...
        """
        if self.debug:
            print("[Debug] Starting interactive chat session")
            
        # Initialize conversation history
        if initial_conversation:
            conversation_history = initial_conversation.copy()
        else:
        ...
        while True:
            try:
                ...
                # Prepare messages based on model support for system role
                if self.model_config.get("supports_system_role", True):
                    ...
                    for i, msg in enumerate(conversation_history):
                        if msg["role"] == "system" and i == 0:
                            # Skip the system message, it will be prepended to first user message
                            continue
                        elif msg["role"] == "user" and len(messages) == 0:
                            # First user message, prepend system instructions
                            combined_content = f"System instructions: {conversation_history[0]['content']}\n\n{msg['content']}"
                ...
                try:
                    ...
                    print(f"\nError: Failed to get response from the model: {str(e)}")
                    if self.debug:
                        print(f"[Debug] Chat error details: {str(e)}")
                    continue
